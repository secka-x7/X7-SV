// X7-SV · vaults.js — 10 SVs · 5,000 instances · shared execution · all 50 chains
...
import { processPropellers, p2Cascade, p3Temporals, p9MultiChain } from './propellers.js'
...
// ── SHARED EXECUTION FUNCTION (all 10 SVs call this) ──────────────────────────
async function execute(chainName, svKey, calldata, profitEst) {
  const contractAddr = getContractAddr(chainName)
  if (!contractAddr) {
    _sv[svKey] && (_sv[svKey].missed += profitEst)
    const total = Object.values(_sv).reduce((s, v) => s + v.missed, 0)
    setConfig('sv_missed_total', total.toFixed(2))
    emit('missed_rev', { chain: chainName, sv: svKey, amount: profitEst })
    return null
  }

  const key = chainName + svKey
  if (_busy[key]) { _sv[svKey] && (_sv[svKey].missed += profitEst); return null }
  _busy[key] = true

  try {
    // Apply propellers to amplify before execution
    const txHash = await executeBundle(chainName, contractAddr, calldata, profitEst)
    if (!txHash) { _sv[svKey] && (_sv[svKey].missed += profitEst); return null }

    _sv[svKey] && (_sv[svKey].total += profitEst, _sv[svKey].count++)
...
async function sweepProfit(chainName, contractAddr) {
  const chain = getChain(chainName)
  if (!chain) return
  const tokens = [chain.weth, chain.wbtc, chain.dai].filter(Boolean)
  const data = encodeFunctionData({
    abi: parseAbi(['function sweep(address[],address) external']),
    functionName: 'sweep', args: [tokens, (await import('./pimlico.js')).getExecutorAddress()]
  })
  await executeBundle(chainName, contractAddr, data, 0).catch(() => {})
}
...
function decodeAmounts(data) {
  ...
  const MAX = BigInt('0x7' + 'f'.repeat(63))
  const FULL = 2n ** 256n
...
const MEGA_POOLS = {
  ethereum: [
    ...
    '0x9a772018FbD77fcD2d25657e5C547BAfF3Db7D2', // USDC/USDT 0.01%
  ],
  arbitrum: [
    '0xC6962004f452bE9203591991D15f6b388e09E8D0',
    '0x2f5e87C9312fa29aed5c179E456625D79015299c',
    '0xA961F0473dA4864C5eD28e00FcC53a3AAb056c1',
  ],
  polygon: [
...
async function onMegaSwap(chainName, log, swapUSD) {
  ...
  for (const opp of cascades) {
    const cascadeData = encodeFunctionData({
      abi: ARB_ABI, functionName: 'dexArb',
      args: [chain.usdc, chain.weth, amountIn / 2n, opp.buyFee, opp.sellFee, 0n]
    })
    await execute(chainName, 'sv2', cascadeData, opp.profitUSD)
  }

  // P9: Multi-chain simultaneous fire
  await p9MultiChain({ swapUSD, buyFee, sellFee }, async (otherChain) => {
    ...
    const oCalldata = encodeFunctionData({
      abi: ARB_ABI, functionName: 'dexArb',
...
function watchChain(chainName) {
  ...
  ws.on('log', async (log) => {
    ...
    if (swapUSD < MIN_SWAP_USD || swapUSD > 2e9) return
...
// Periodic arbs for chains without explicit pool lists (Tier 2/3)
async function periodicArb(chainName) {
  ...
  const opp = await processPropellers(chainName, {
    tokenIn: chain.usdc, tokenOut: chain.weth, amountIn: amount,
...
// SV-7: Stablecoin arb
async function sv7Stable(chainName) {
  const chain = getChain(chainName)
  if (!chain?.usdc || !chain?.dai) return
  const opp = await processPropellers(chainName, {
    tokenIn: chain.usdc, tokenOut: chain.dai, amountIn: BigInt(1e11), buyFee: 100, sellFee: 500, profitEst: 50
  })
  const calldata = encodeFunctionData({
    abi: ARB_ABI, functionName: 'dexArb',
    args: [opp.tokenIn, opp.tokenOut, opp.amountIn, opp.buyFee, opp.sellFee, 0n]
...
export function startVaults() {
  console.log('[VAULTS] Starting 10 SVs · 5,000 instances · $100M+ targets')

  const all = getActiveChains()

  // Watch pools on chains that have pool lists
  all.forEach(c => watchChain(c.name))

  // Periodic arbs: Tier1 every 2s, Tier2 every 5s, Tier3 every 15s
  const intervals = { 1: 2000, 2: 5000, 3: 15000 }
  ;[1, 2, 3].forEach(tier => {
    const chains = getTierChains(tier)
    setInterval(async () => {
      for (const c of chains) {
        await periodicArb(c.name).catch(() => {})
        await sv7Stable(c.name).catch(() => {})
        await new Promise(r => setTimeout(r, 100))
      }
    }, intervals[tier])
  })

  console.log(`[VAULTS] Live on ${all.length} chains`)
}
